...
class ProjectAnalysisPromptBuilder(PromptBuilder):
    SYSTEM_PROMPT = (
        ...
        "    artifact_relationships = read_artifact_json('relationships.json')\n"
        "    source_paths = list(repo_map['source_worklist'])\n"
        "    artifact_docs = artifact_manifest.get('documents', [])\n"
        "    relationships = artifact_relationships.get('relationships', [])\n"
        "    important_paths = []\n"
        ...
        "source_doc_snippets = []\n"
        "for doc in artifact_docs[:8]:\n"
        "    artifact_path = doc.get('artifact_path')\n"
        "    if artifact_path:\n"
        "        source_doc_snippets.append({'path': doc.get('source_path'), 'snippet': read_artifact(artifact_path, 0, 800)})\n"
        "details = []\n"
