...
class TestProjectAnalysisContract(unittest.TestCase):
    ...
    def test_project_analysis_prompt_contains_valid_controller_code_example(self):
        ...
        self.assertIn("artifact_relationships = read_artifact_json('relationships.json')", prompt)
        self.assertIn("artifact_docs = artifact_manifest.get('documents', [])", prompt)
        self.assertIn("relationships = artifact_relationships.get('relationships', [])", prompt)
    ...
    def test_controller_reads_worker_artifacts_for_synthesis(self):
        analyzer, _ = self._make_analyzer(
            [
                ...
                "finish({'summary': 'Controller synthesized from artifact ' + hit['path'] + ' after reading manifest with source-docs present: ' + str(manifest['documents'][0]['artifact_path'] == 'source-docs/app.py.md')})"
            ],
            max_steps=1,
        )
